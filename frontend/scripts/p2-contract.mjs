...
const checks = [
  ...
  [authClient.includes("csrfTokenCache = null;") && authClient.includes("signIn("), "frontend invalidates cached CSRF token after login rotation"],
  ...
  [rbac.includes("if not matched:\n        return VIEWER") && rbac.includes("is_superuser") && rbac.includes("return ADMIN"), "legacy P2 group role remains deterministic for bootstrap/migration compatibility"],
  ...
  [authViews.includes("authorization_for_membership") || authViews.includes("authorization_for_user(user)"), "auth/me derives role and capabilities on the server"],
  [roleCommand.includes("set_domaintwin_role") === false && roleCommand.includes("ROLE_GROUPS") && roleCommand.includes("user.groups.remove") && roleCommand.includes("user.groups.add"), "management command replaces a user's legacy DomainTwin group role deterministically"],
